...
export const generateAdminNotesPDF = async (notes, pdfTitle = 'Private Admin Notes') => {
  try {
    const { jsPDF } = await import('jspdf');
    const logoImg = await loadLogoImage();
    ...
    const prefix = "Copyright © 2026 CodeDiary | Admin Export | ";
    const linkText = "LinkedIn";
    doc.setFontSize(8.5);
    doc.setFont('helvetica', 'normal');
    doc.setTextColor(128, 128, 128);
    ...
    if (notesList.length === 0) {
      ...
      doc.setFontSize(11);
      doc.setTextColor(95, 99, 104);
      doc.text("No private notes available.", margin, y);
    } else {
      for (let idx = 0; idx < notesList.length; idx++) {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(13);
        doc.setTextColor(26, 115, 232); // Blue accent
        doc.text(`${notesList.length > 1 ? `${idx + 1}. ` : ''}${note.title}`, margin, y);
        y += 6;
        ...
        if (note.created_at || note.updated_at) {
          ...
          doc.setFontSize(8);
          doc.setTextColor(128, 134, 139);
          const noteDate = new Date(note.created_at || note.updated_at).toLocaleString();
          doc.text(`Created / Updated: ${noteDate}`, margin, y);
          y += 5;
        }
        ...
        if (notesList.length > 1 && idx < notesList.length - 1) {
          checkPageBreak(8);
    ...
    console.error("PDF generation for admin notes failed:", error);
    return null;
  }
};
