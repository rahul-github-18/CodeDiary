...
export const todoService = {
  ...
  getModuleBySlug: async (categorySlug, topicSlug) => {
    const res = await fetch(`/api/modules/${encodeURIComponent(categorySlug)}/${encodeURIComponent(topicSlug)}?t=${Date.now()}`, {
      headers: getAuthHeaders(),
      cache: 'no-store'
    });
    return handleResponse(res);
...
export const noteService = {
  ...
  createNote: async (topicId = null, data) => {
    const res = await fetch('/api/notes', {
      method: 'POST',
      headers: getAuthHeaders(),
      body: JSON.stringify({ topic_id: topicId || null, ...data })
    });
    return handleResponse(res);
  },
...
export const certificateService = {
  getCertificateByTopic: async (topicId) => {
    const res = await fetch(`/api/certificates/topic/${topicId}?t=${Date.now()}`, {
      headers: getAuthHeaders(),
      cache: 'no-store'
    });
  ...
  verifyCertificate: async (code) => {
    const res = await fetch(`/api/certificates/verify/${code}?t=${Date.now()}`, {
      cache: 'no-store'
    });
    if (res.status === 404) return null;
    return handleResponse(res);
