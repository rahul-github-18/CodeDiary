...
export default function AboutPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 font-sans selection:bg-sky-500 selection:text-white relative overflow-hidden">
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20 relative z-10">
        ...
        <section className="text-center max-w-3xl mx-auto mb-20">
          <div className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-800/80 border border-slate-700/80 backdrop-blur-md text-xs font-semibold text-sky-400 mb-6 shadow-sm">
          ...
          <div className="flex items-center justify-center gap-4 mb-6">
            <img 
              src="/light-logo.png" 
              alt="CodeDiary Logo" 
              className="w-14 h-14 rounded-2xl object-contain bg-white p-2 shadow-lg shadow-sky-500/10 border border-slate-700" 
            />
            <h1 className="text-4xl sm:text-5xl font-extrabold tracking-tight text-white leading-tight">
              About <span className="text-transparent bg-clip-text bg-gradient-to-r from-sky-400 via-indigo-400 to-emerald-400">CodeDiary</span>
            </h1>
          </div>

          <p className="text-base sm:text-lg text-slate-300 leading-relaxed font-normal mb-8">
            CodeDiary is a modern, structured developer workspace built for learning programming, 
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <Link
              ...
              className="px-6 py-3 rounded-full bg-sky-500 hover:bg-sky-400 text-white font-semibold text-sm transition-all shadow-lg shadow-sky-500/25 hover:shadow-sky-500/40 transform hover:-translate-y-0.5"
            >
              Get Started Free
            </Link>
            <Link
              href="/share-code"
              className="px-6 py-3 rounded-full bg-slate-800 hover:bg-slate-700 border border-slate-700 text-slate-200 font-semibold text-sm transition-all shadow-sm"
            >
              Try Share Code
            </Link>
          </div>
        ...
        <section className="mb-24">
          <div className="bg-slate-800/50 border border-slate-700/60 rounded-3xl p-8 sm:p-12 backdrop-blur-md relative overflow-hidden shadow-xl">
            <div className="absolute top-0 right-0 w-64 h-64 bg-sky-500/5 rounded-full blur-2xl pointer-events-none" />
        ...
        <section className="mb-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((feature, idx) => (
              <article 
                key={idx}
                className="bg-slate-800/40 border border-slate-700/50 hover:border-slate-600/80 rounded-2xl p-6 transition-all duration-200 hover:-translate-y-1 hover:shadow-xl hover:shadow-sky-500/5 flex flex-col justify-between"
              >
                <div>
                  <div className="w-12 h-12 rounded-xl bg-slate-800 border border-slate-700/80 flex items-center justify-center mb-4">
                    {feature.icon}
                  </div>
        ...
        <section className="mb-24">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {audience.map((item, idx) => (
              <div 
                key={idx} 
                className="bg-slate-800/30 border border-slate-700/40 rounded-xl p-5 hover:bg-slate-800/60 transition-colors"
              >
                <div className="w-2 h-2 rounded-full bg-sky-400 mb-3" />
        ...
        {/* 5. Technology Stack */}
        <section className="mb-24">
          <div className="bg-slate-800/40 border border-slate-700/60 rounded-3xl p-8 sm:p-12">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {techStack.map((tech, idx) => (
                <div 
                  key={idx} 
                  className="bg-slate-900/60 border border-slate-700/50 rounded-2xl p-5 flex flex-col justify-between"
                >
                  <span className="text-[11px] font-semibold tracking-wider text-sky-400 uppercase mb-2">
                    {tech.category}
                  </span>
        ...
        <section className="mb-24 text-center max-w-3xl mx-auto">
          <div className="p-8 sm:p-10 rounded-3xl bg-gradient-to-b from-slate-800/80 to-slate-800/30 border border-slate-700/70 shadow-2xl">
        ...
        <section className="mb-24">
          ...
          <div className="max-w-3xl mx-auto space-y-4">
            {faqs.map((faq, idx) => (
              <details 
                key={idx} 
                className="group bg-slate-800/40 border border-slate-700/50 rounded-2xl p-6 [&_summary::-webkit-details-marker]:none cursor-pointer transition-all duration-200 hover:border-slate-600/80"
              >
                <summary className="flex items-center justify-between font-bold text-base text-white">
                  <span>{faq.q}</span>
                  <span className="ml-4 transition-transform duration-200 group-open:rotate-180 text-sky-400">
        ...
        {/* 8. Footer */}
        <footer className="border-t border-slate-800 pt-12 pb-6 text-center text-xs text-slate-500">
          <div className="flex flex-wrap items-center justify-center gap-6 mb-6 font-medium text-slate-400">
            <Link href="/" className="hover:text-white transition-colors">Home</Link>
            <Link href="/about" className="hover:text-white transition-colors">About</Link>
            <Link href="/share-code" className="hover:text-white transition-colors">Share Code</Link>
            <Link href="/login?mode=login" className="hover:text-white transition-colors">Login</Link>
            <Link href="/login?mode=enroll" className="hover:text-white transition-colors">Enroll</Link>
          </div>
